...
class QueryRequest(BaseModel):
    ...
    clearance: Optional[str] = Field(None, description="Clearance explicite transmise par le backend")
...
async def ingest_documents(payload: IngestRequest):
    """
    Ingests document chunks directly into ChromaDB via the RetrievalAgent.
    """
    try:
        collection = orchestrator.retrieval_agent.collection
        
        ids = [str(uuid.uuid4()) for _ in payload.chunks]
        metadatas = [
            {
        ...
        return {"status": "success", "inserted": len(payload.chunks)}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
